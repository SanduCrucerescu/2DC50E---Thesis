namespace HeapSort3; // was not created I did it manually
...
class HeapSort
{
    ...
    static void TDHeapSort(List<object> aList, int aFirst, int aLast, TtdCompareFunc aCompare)
    {
        ...
        for (int Inx = ItemCount / 2 - 1; Inx >= 0; Inx--)
            HSTrickleDownStd(aList.ToArray(), aFirst + Inx, ItemCount, aCompare);
        ...
        for (int Inx = ItemCount - 1; Inx >= 0; Inx--)
        {
            ...
            HSTrickleDown(aList.ToArray(), aFirst, Inx, aCompare);
        }
    }
