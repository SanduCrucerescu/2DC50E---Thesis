namespace HeapSort4; // was not created I did it manually
...
{
    internal static void TDValidateListRange(IList<object> aList, int aStart, int aEnd, string aMessage)
    {
        if (aList == null)
    ...
    internal static void HSTrickleDown(object[] aList, int aFromInx, int aCount, TtdCompareFunc aCompare)
    {
        ...
        {
            if ((ChildInx + 1 < aCount) &&
                (aCompare(aList[ChildInx], aList[ChildInx + 1]) < 0))
                ChildInx++;
    ...
    internal static void TDHeapSort(IList<object> aList, int aFirst, int aLast, TtdCompareFunc aCompare)
    {
        TDValidateListRange(aList, aFirst, aLast, "TDHeapSort");
        int ItemCount = aLast - aFirst + 1;
        for (int Inx = ItemCount / 2 - 1; Inx >= 0; Inx--)
            HSTrickleDownStd(aList.ToArray(), Inx, ItemCount, aCompare);
        ...
        for (int Inx = ItemCount - 1; Inx >= 0; Inx--)
        {
            ...
            HSTrickleDown(aList.ToArray(), 0, Inx, aCompare);
        }
    }
