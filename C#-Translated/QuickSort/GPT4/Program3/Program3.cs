...
class Program
{
    ...
    internal static void TDValidateListRange(List<int> list, int start, int end, string message)
    {
        if (list == null)
    ...
    internal static void QSS(List<int> list, int first, int last, Func<int, int, int> compare)
    {
        int l, r;
        int pivot;
        int temp;
    ...
    internal static void TDQuickSortStd(List<int> list, int first, int last, Func<int, int, int> compare)
    {
        TDValidateListRange(list, first, last, "TDQuickSortStd");
        QSS(list, first, last, compare);
    }

    internal static int TDCompareLongint(int data1, int data2)
    {
        if (data1 < data2)
            return -1;
        else if (data1 == data2)
